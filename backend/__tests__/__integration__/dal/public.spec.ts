...
describe("PublicDAL", function () {
  it("should be able to update stats", async function () {
    ...
    await PublicDAL.updateStats(1, 15, 3);
  });
  ...
  it("should be able to get typing stats", async function () {
    ...
    expect(typingStats).toHaveProperty("totalWordsTyped");
  ...
  it("should increment stats on update", async function () {
    ...
    await PublicDAL.updateStats(1, 60, 12);
    ...
    expect(afterStats.totalWordsTyped).toBe(
      (priorStats.totalWordsTyped ?? 0) + 12,
    );
